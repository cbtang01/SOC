...
class XCSAlgorithm(LCSAlgorithm):
    ...
    def cover(self, match_set):
        ...
        action_candidates = (
            ...
            match_set.model.possible_actions.difference(match_set)
    ...
    def update(self, match_set):
        ...
        action1, action2 = parent1.action, parent2.action
        action1 = self._mutate_action(action1, action_set.action)
        action2 = self._mutate_action(action2, action_set.action)
        ...
        for condition in condition1, condition2:
            ...
            if self.do_ga_subsumption:
                ...
                if subsumed:
                    continue
            if parent1.prediction >= parent2.prediction:
                action = action1
            else:
                action = action2
    ...
    def _mutate_action(self, action, classification):
        """Create a new action from the given one by probabilistically
        applying point-wise mutations. Bits that were originally wildcarded
        in the parent action acquire their values from the provided
        classification, to ensure the child action continues to match it."""

        # Go through each position in the action, randomly flipping
        # whether the position is a value (0 or 1). We do
        ...
        mask = action.__xor__(mutation_points)

        # The bits that aren't wildcards always have the same value as the
        # classification, which ensures that the mutated action still matches
        # the classification.

        return classification.__and__(mask)
